...
```{r prerequisites, echo=FALSE}
        ...
        path <- "GIT\\RCT-adjusting-for-covariates"     
...
```{r echo=TRUE, eval=TRUE, tidy=FALSE, results='hide' }     
          ...
          # was initially using map and not map2stan
          # seems to be important that enough iterations are run 
          # otherwise there is an error and only default 1000
          # simulations are generated and the rest NAs
...
```{r echo=TRUE, results='hide',   fig.cap="Predicted effects (Statistical Rethinking page 129)", fig.show='asis'}     
      ...
      mu1 <- link( m1 , data=pred.data1 , n=simulations)  # default is 1000 
      mu.mean <- apply( mu1 , 2 , mean )
      mu.PI <- apply( mu1 , 2 , PI )
 
      # simulate counterfactual response outcomes
      R.sim1 <- sim( m1 , data=pred.data1 , n=simulations )  
      ...
      mu2 <- link( m1 , data=pred.data2 , n =simulations)
      mu.mean <- apply( mu2 , 2 , mean )
      mu.PI <- apply( mu2 , 2, PI )
  
      # simulate counterfactual response outcomes
      R.sim2 <- sim( m1 , data=pred.data2 , n=simulations )
...
```{r echo=TRUE, fig.cap="Predicted effects (Statistical Rethinking page 129)"   }
        ...
        shade( mu.PI , P.biomarker , col= col.alpha("blue", 0.15))
        shade( R.PI ,  P.biomarker ,  col= col.alpha("purple", 0.15))
        x <- sample(1:simulations,1)      # choose one simulated dataset to plot
...
```{r echo=TRUE, results='hide'}
      ...
      m2 <- map2stan( alist( Y ~ dnorm(mu, sigma), 
...
```{r echo=TRUE, fig.cap="Check chains are mixing" }  
   ...
   # par(mfrow=c(2,4))
  #  for ( i in 1:8 )  { plot(post[i] , type='l', main=names(post)[i]) } #post[,i]
  #  par(mfrow=c(1,1))
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          # compute counterfactual mean response (mu1)
          mu1 <- link( m1 , data=pred.data1  , n=simulations )   
          mu.mean <- apply( mu1 , 2 , mean )
          mu.PI <- apply( mu1 , 2 , PI )
          ...
          mtext(paste(
            "Adjusted to treatment",P.treat,"; biomarker", P.biomarker,"; Sex",P.sex,"; Baseline",p4(P.baseline),
            sep=" "))
          lines( x , mu.mean )
...
#         sep=" "))
      ...
      #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      P.biomarker <- median(d$biomarker)                   # Hold
      P.baseline <- median(d$baseline)                     # Hold at median
      P.sex <- c(rep(0,75) , rep(1, 75))                   # Hold sex at FEMALE
      P.age <- median(d$age)                               # Hold age at median 
      P.treat=1                                            # Hold
